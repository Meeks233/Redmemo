...
func TestAuthenticate_Password(t *testing.T) {
	var gotGrant, gotUser, gotAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"access_token":"pw-tok","expires_in":3600}`))
	}))
	defer srv.Close()

	c := newClientToServer(t, srv)
	res, err := c.Authenticate(config.OAuthTokenConfig{
		Backend:      "password",
		Username:     "alice",
		Password:     "hunter2",
		ClientID:     "cid",
	...
	wantAuth := "Basic " + basicAuth("cid", "csecret")
	if gotAuth != wantAuth {
		t.Errorf("Authorization = %q, want %q", gotAuth, wantAuth)
	}
	if res.Identity.UserAgent == "" {
		t.Error("password auth should set a User-Agent identity")
	}
}
